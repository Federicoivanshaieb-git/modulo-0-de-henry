...
function generarContrasena(longitud, incluirEspeciales, incluirNumeros, incluirMayusculas ) {
  ...
  var letrasMayusculas = letras.toLocaleUpperCase()
...
function cajaFuerte(codigoSecreto, cantidadIntentos){
  ...
  if ( cantidadIntentos < 1 || cantidadIntentos >5) return "Solo se permite una cantidad de intentos mayor a 0 y menor a 6 "
...
function desbloquearCajaFuerte(codigoSecreto, cantidadIntentos, codigoDesbloqueo){
  ...
  for (var i=0; i < codigoDesbloqueo.length; i++){
    if(isNaN(codigoDesbloqueo[i]))return "El codigo secreto solo puede estar conformado por numeros";
  }
...
function eliminarActividad(indice){
    if (isNaN (indice)) return "El indice no es valido, debe de ser un numero";
    if (indice<0||indice>actividadesSospechozas.length-1) return "El indice no es valido, se encuentra fuera del rango"
...
function filtrarActividadesPorRiesgo(nivelRiesgo){
    ...
    var actividadesConFiltro = actividadesSospechozas.filter(function(actividad){
        return actividad.includes(`Riesgo- ${nivelRiesgo}`)
    })
    if(actividadesConFiltro === 0) return "no hay actividades para mostrar"
    
    return actividadesConFiltro;
...
function generarReporteDeActividades(){
    /* TU CODIGO */
    var actividadcreada = actividadesSospechozas.map(function(actividad,indice){
        return `Id: ${indice} , ${actividad}`
    })
    if(actividadcreada.length === 0) return  "No hay actividades para mostrar"
...
var asistente = {
    ...
    if (opcion==="nombre") return perfiles.map(function(elemento){
        return elemento.usuario
    })
    if (opcion==="nivel") return perfiles.map(function(elemento){
        return elemento.nivel_de_autorizacion
    })
    else return perfiles.map(function(elemento){
        return elemento[opcion]
    })
    },
    
    verPerfilesPorAntiguedad: function(){
        /* TU CODIGO */
        var verPerfilesPorAntiguedad=[...perfiles]
            return verPerfilesPorAntiguedad.sort(function(a,b){
    ...
    verAdministradores: function(){
        /* TU CODIGO */
        var admin=perfiles.filter(function(elemento){
            return  elemento.nivel_de_autorizacion==="admin"
        }) 
        return admin;
    },
    ...
    modificarAcceso: function(usuario, codigo){
        /* TU CODIGO */
        var encontrarUsuario=perfiles.find(function(elemento){
            return elemento.usuario === usuario
        })
        if (!encontrarUsuario) return "usuario no encontrado"
        if (codigo.length !==4 || isNaN(codigo)) return "codigo de acceso invalido, debe de contener solo 4 numeros"
            encontrarUsuario.codigo=Number(codigo)
